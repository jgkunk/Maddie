...
#__t1__ __s1_    ___t2__  __s2__   __t3___ __s3___ 
#             e1                 e2                
#  1:12,13:22,NA,24:35,36:45,NA,47:58,59:68
#  1:12,13:22,--,23:34,35,44,--,45:56,57:66
isna=(1:(length(V1)))[is.na(V1)]
 b1=isna[1]; if (length(isna)>2) {e1=isna[2]; b2=isna[3]; e2=isna[4]; b3=isna[5]} else {e1=b1; b2=isna[2]; e2= b2; b3=length(V1)} # +1

t1<- 1:12;       s1<- 13:(b1-1);  if(e1>b1) { v1<-(b1+1):(e1-1) ; } else {v1=NULL }
t2<- e1+t1;      s2<- (max(t2)+1):(b2-1); if(e1>b1) {v2<-(b2+1):(e2-1) ;}  else {v2=NULL }
t3<- e2+t1;      s3<- (max(t3)+1):(b3-1); if(e1>b1) {v3<-(b3+1):(length(V1)) ;} else {v3=NULL }
...
t1<- 1:12;         s1<- 13:(b1-1);          if(e1>b1) v1<-(b1):(e1-2) else v1=NULL 
if(e1>b1) {t2<- e1-2+t1; s2<- (max(t2)+1):(b2-3);  v2<-(b2-2):(e2-4) } else {t2<- e1-1+t1; s2<- (max(t2)+1):(b2-1);v2=NULL }
if(e1>b1) {t3<- e2-4+t1; s3<- (max(t3)+1):(b3-5);  v3<-(b3-4):(length(V1)-5) } else {t3<- e2-2+t1; s3<- (max(t3)+1):(b3-2);v3=NULL }
detach(XX)
quartz(width=12, height=3)   # was 8 and 2
par(mar=c(0.5,0.5,1,0.2))
sep=1.2
AngZ<- 0   # was -0.1
AngX<- 0.5*pi    # was 0.5 or 0.5*pi
